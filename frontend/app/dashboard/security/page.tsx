...
export default function SecurityCenterPage() {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold font-display text-slate-900">Security Center</h1>
        <p className="text-sm text-slate-500">AI-powered cloud security and threat detection.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* Security Score */}
        <div className="glass-card rounded-2xl p-6 border-white/80 shadow-sm flex flex-col justify-between bg-gradient-to-br from-white to-slate-50">
          <div>
            <h2 className="text-lg font-bold text-slate-900">Security Score</h2>
            <p className="text-xs text-slate-500">Based on active threats and configs.</p>
          </div>

          <div className="text-center my-6">
            <div className="text-6xl font-bold text-emerald-600 font-display">A+</div>
            <p className="text-sm text-slate-600 font-medium mt-1">Excellent Protection</p>
          </div>
        ...
        <div className="lg:col-span-2 glass-card rounded-2xl p-6 border-white/80 shadow-sm">
          <div className="flex justify-between items-center mb-4">
            <div>
              <h2 className="text-lg font-bold text-slate-900">Recent Threats</h2>
              <p className="text-xs text-slate-500">Flagged by AI engine.</p>
            </div>
          ...
          <div className="space-y-3">
            {threats.map((threat) => (
              <div key={threat.id} className="bg-white border border-slate-100 rounded-xl p-3 flex justify-between items-center text-xs">
                <div className="flex items-center gap-2">
                  <div className={`p-1.5 rounded-lg ${threat.severity === 'critical' ? 'bg-rose-50 text-rose-600' : threat.severity === 'high' ? 'bg-amber-50 text-amber-600' : 'bg-blue-50 text-blue-600'}`}>
                    <ShieldAlert size={14} />
                  </div>
                  <div>
                    <span className="font-bold text-slate-800">{threat.type}</span>
                    <span className="text-slate-500 block">Source: {threat.source}</span>
                  </div>
                </div>
        ...
        <div className="lg:col-span-3 glass-card rounded-2xl p-6 border-white/80 shadow-sm">
          <div className="flex justify-between items-center mb-4">
            <div>
              <h2 className="text-lg font-bold text-slate-900">Attack Map</h2>
              <p className="text-xs text-slate-500">Real-time visualization of blocked attempts.</p>
            </div>
            <div className="flex items-center gap-2">
