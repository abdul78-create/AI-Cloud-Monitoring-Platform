      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="glass-card rounded-2xl p-6 border-white/80 dark:border-slate-800 shadow-sm flex flex-col justify-between">
          ...
          <div className="space-y-4 my-4">
            <div className="flex justify-between items-center">
              ...
              <span className="font-bold text-slate-900 dark:text-white">69</span>
            </div>
            <div className="flex justify-between items-center">
              <div className="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-400">
          ...
          <div className="text-xs text-slate-500 dark:text-slate-400 text-center font-medium">
            Overall Success Rate: <span className="text-emerald-600 dark:text-emerald-400 font-bold">94.5%</span>
          </div>
        </div>
        ...
        <div className="lg:col-span-3 glass-card rounded-2xl p-6 border-white/80 dark:border-slate-800 shadow-sm">
          ...
          <div className="space-y-2">
            {deployments.map((dep) => (
              <div key={dep.id} className="bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-xl p-3 flex justify-between items-center text-xs">
                <div className="flex items-center gap-3">
                  <div className={`p-1.5 rounded-lg ${dep.status === 'success' ? 'bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600' : 'bg-rose-50 dark:bg-rose-900/30 text-rose-600'}`}>
                    <Box size={14} />
                  </div>
                  <div>
                    <span className="font-bold text-slate-800 dark:text-white">{dep.version}</span>
